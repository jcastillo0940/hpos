...
@section('page-title', 'Nueva Bodega')
...
<div class="mb-6">
    <div class="flex items-center space-x-2 text-sm text-slate-600 mb-4">
        ...
        <span class="text-slate-800 font-medium">Nueva Bodega</span>
    </div>
    <h3 class="text-2xl font-bold text-slate-800">Crear Nueva Bodega</h3>
</div>

<form action="{{ route('bodegas.store') }}" method="POST" class="max-w-4xl">
    @csrf
    ...
    <div class="bg-white rounded-xl shadow-sm p-6 mb-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <select name="tipo" required class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent @error('tipo') border-red-500 @enderror" x-data="{ tipo: '{{ old('tipo', 'Principal') }}' }" x-model="tipo" @change="$dispatch('tipo-changed', tipo)">
            ...
            <div x-data="{ showPlaca: '{{ old('tipo') }}' === 'Móvil' }" @tipo-changed.window="showPlaca = $event.detail === 'Móvil'">
                <label class="block text-sm font-medium text-slate-700 mb-2">
                    Placa del Vehículo
                    <span x-show="showPlaca" class="text-red-500">*</span>
                </label>
                <input type="text" name="placa_vehiculo" value="{{ old('placa_vehiculo') }}" :required="showPlaca" :disabled="!showPlaca" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent @error('placa_vehiculo') border-red-500 @enderror" :class="!showPlaca && 'bg-gray-100'">
            ...
            <div class="md:col-span-2">
                ...
                <textarea name="direccion" rows="2" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent @error('direccion') border-red-500 @enderror">{{ old('direccion') }}</textarea>
                @error('direccion')
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                @enderror
            </div>
    ...
    <div class="bg-white rounded-xl shadow-sm p-6 mb-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
                ...
                <input type="text" name="responsable" value="{{ old('responsable') }}" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent @error('responsable') border-red-500 @enderror">
                @error('responsable')
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <input type="text" name="telefono" value="{{ old('telefono') }}" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent @error('telefono') border-red-500 @enderror">
                @error('telefono')
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                @enderror
            </div>

            <div x-data="{ showRepartidor: '{{ old('tipo') }}' === 'Móvil' }" @tipo-changed.window="showRepartidor = $event.detail === 'Móvil'">
                <label class="block text-sm font-medium text-slate-700 mb-2">
                    Repartidor Asignado
                </label>
    ...
    <div class="bg-white rounded-xl shadow-sm p-6 mb-6">
        ...
        <div class="flex items-center">
            <label class="relative inline-flex items-center cursor-pointer">
                <input type="checkbox" name="activa" value="1" {{ old('activa', true) ? 'checked' : '' }} class="sr-only peer">
