...
@section('content')
<div class="max-w-7xl mx-auto" x-data="clienteEditForm()">
    <div class="bg-white rounded-xl shadow-sm p-6">
        <form method="POST" action="{{ route('clientes.update', $cliente) }}" @submit="validateForm">
            @csrf
            @method('PUT')
            ...
            <div class="mb-8">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div>
                        ...
                        <input type="text" name="codigo" required value="{{ old('codigo', $cliente->codigo) }}" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                    </div>
                    ...
                    <div>
                        ...
                        <select name="tipo_identificacion" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                            <option value="ruc" {{ $cliente->tipo_identificacion == 'ruc' ? 'selected' : '' }}>RUC</option>
                            <option value="cedula" {{ $cliente->tipo_identificacion == 'cedula' ? 'selected' : '' }}>Cédula</option>
                    ...
                    <div>
                        ...
                        <input type="text" name="identificacion" required value="{{ old('identificacion', $cliente->identificacion) }}" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                    </div>

                    <div>
                        <label class="block text-sm font-medium text-slate-700 mb-2">Razón Social</label>
                        <input type="text" name="razon_social" value="{{ old('razon_social', $cliente->razon_social) }}" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                    </div>

                    <div>
                        <label class="block text-sm font-medium text-slate-700 mb-2">Nombre Comercial *</label>
                        <input type="text" name="nombre_comercial" required value="{{ old('nombre_comercial', $cliente->nombre_comercial) }}" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                    </div>

                    <div>
                        <label class="block text-sm font-medium text-slate-700 mb-2">Email</label>
                        <input type="email" name="email" value="{{ old('email', $cliente->email) }}" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                    </div>

                    <div>
                        <label class="block text-sm font-medium text-slate-700 mb-2">Teléfono</label>
                        <input type="text" name="telefono" value="{{ old('telefono', $cliente->telefono) }}" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                    </div>

                    <div class="md:col-span-2">
                        <label class="block text-sm font-medium text-slate-700 mb-2">Dirección Principal</label>
                        <textarea name="direccion" rows="2" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent">{{ old('direccion', $cliente->direccion) }}</textarea>
                    </div>
                </div>
            </div>
            ...
            <div class="mb-8">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    ...
                    <div>
                        ...
                        <select name="zona_id" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                            ...
                            @foreach($zonas as $zona)
                                <option value="{{ $zona->id }}" {{ $cliente->zona_id == $zona->id ? 'selected' : '' }}>{{ $zona->nombre }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div>
                        ...
                        <select name="ruta_id" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                            ...
                            @foreach($rutas as $ruta)
                                <option value="{{ $ruta->id }}" {{ $cliente->ruta_id == $ruta->id ? 'selected' : '' }}>{{ $ruta->nombre }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div>
                        ...
                        <select name="lista_precio_id" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                            ...
                            @foreach($listaPrecios as $lista)
                                <option value="{{ $lista->id }}" {{ $cliente->lista_precio_id == $lista->id ? 'selected' : '' }}>{{ $lista->nombre }} ({{ $lista->tipo_ajuste == 'porcentaje' ? $lista->valor_ajuste . '%' : 'B/. ' . number_format($lista->valor_ajuste, 2) }})</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div>
                        ...
                        <input type="number" step="0.01" name="limite_credito" value="{{ old('limite_credito', $cliente->limite_credito) }}" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                    </div>

                    <div>
                        <label class="block text-sm font-medium text-slate-700 mb-2">Días de Crédito</label>
                        <input type="number" name="dias_credito" value="{{ old('dias_credito', $cliente->dias_credito) }}" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                    </div>
                </div>
            </div>
                    ...
                    <template x-for="(sucursal, index) in sucursales" :key="index">
                        <div class="border border-gray-200 rounded-lg p-4 bg-gray-50">
                            <div class="flex justify-between items-start mb-3">
                                <h4 class="font-medium text-slate-700" x-text="sucursal.nombre || 'Sucursal ' + (index + 1)"></h4>
                                <button type="button" @click="eliminarSucursal(index)" class="text-red-600 hover:text-red-800">
                                    <i class="bi bi-trash"></i>
                                </button>
                            </div>

                            <input type="hidden" :name="'sucursales['+index+'][id]'" x-model="sucursal.id">
            ...
            <div class="flex justify-end space-x-3">
                ...
                <button type="submit" class="px-6 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg transition shadow-sm hover:shadow-md">
                    <i class="bi bi-check-circle mr-2"></i>Actualizar Cliente
                </button>
            </div>
        </form>
    </div>
...
function clienteEditForm() {
    return {
        sucursales: @json($sucursalesData),
